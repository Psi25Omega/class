...
#' }
#' @export
CLASS2 <- function(X = NULL, y = NULL, csv = NULL, header = FALSE, nSample = -1, nTimes = -1, k = -1) {
  if (nSample == -1) {
  ...
  full_data_lasso <- glmnet::cv.glmnet(x = X, y = y, alpha = 1)
  use_lambda_min <- full_data_lasso$lambda.min
  ...
  for(i in 1:nTimes) {
    ...
    fit <- glmnet::glmnet(x = data_sub$X_subsampled, y = data_sub$y_subsampled, alpha = 1)
    coefs <- coef(fit, s = use_lambda_min)[-1]
